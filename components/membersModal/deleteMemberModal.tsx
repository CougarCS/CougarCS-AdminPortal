  ...
  return (
    ...
    >
      <div className="h-fit w-[30rem] rounded-md bg-sidebarBG p-7 text-white">
        <h1 className="text-4xl">Deleting Contact</h1>
        ...
        <p className="mb-2 pb-2 text-lg">
          Type <span className="font-bold">{member.first_name}</span> into the
        ...
        <input
          ...
          className={
            "mb-0 h-10 w-full rounded-md border-[1px] border-gray-600 bg-transparent px-3 outline-0 transition-all hover:border-gray-200 focus:border-gray-200"
          }
        ></input>

        <button
          disabled={member.first_name !== nameConfirm}
          ...
          onClick={async () => {
            ...
            if (delResp.error) {
              setModalOpen(false);
              toast.error(
                `Failed to delete ${member.first_name}: ${delResp.error}`
              );
              return;
            }
            setModalOpen(false);
